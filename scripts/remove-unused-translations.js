 ...
 * - Removes only keys that contain a dot (.) by default, because
 *   non-dotted keys are commonly used via dynamic lookups (t(variable)).
...
function findPropertyRanges(source, key) {
  // Returns an array of { start, end } ranges to remove.
  // Uses a lightweight scanner to find the comma that terminates the property
  ...
  while ((match = keyRegex.exec(source)) !== null) {
    ...
    let i = keyRegex.lastIndex; // points just after the ':'

    // scan forward to find the end of the property (comma at depth 0)
    let depthParen = 0;
    let depthBracket = 0;
    let depthBrace = 0;
    let inString = null; // one of ', ", `
    let escaped = false;
    ...
    for (; i < source.length; i += 1) {
      ...
      if (ch === '(') depthParen += 1;
        ...
        if (depthBrace === 0) {
          // likely end of the object literal. property may be last without trailing comma.
...
function applyRanges(source, ranges) {
  ...
  // Clean up: collapse triple newlines caused by removals
  out = out.replace(/\n{3,}/g, '\n\n');
  return out;
}
...
function main() {
  ...
  const keysToRemove = candidates
    ...
    .filter((k) => {
      ...
      // Default safe path: only structured dotted keys.
      if (!args.includeNondotted) {
        return hasDot && !hasWhitespace;
      }

      // With non-dotted enabled, still keep dotted keys (same safety rule).
      if (hasDot) return !hasWhitespace;

      // Non-dotted keys are high risk; be conservative unless explicitly overridden.
      if (args.includeNondottedAll) return true;

      // Safe-ish subset: multi-word UI strings, not ALLCAPS.
  ...
  for (const filename of languageFiles) {
    const abs = path.resolve(languagesDir, filename);
    if (!fs.existsSync(abs)) {
      console.warn(`Skipping missing language file: ${rel(repoRoot, abs)}`);
      continue;
    }
    ...
    perFileRemoved[rel(repoRoot, abs)] = removedHere;
