...
function renderAscension() {
  ...
  const upgrades = Object.entries(cat.upgrades)
    .map(([key, cfg]) => {
      ...
      return `<li data-key="${key}"><span class="ascension-upgrade-label"><span class="ascension-upgrade-level">${t('ascension.upgrade.lvl')} ${levelText}</span> ${nameWithBonus}</span> <button class="ascension-upgrade-btn" ${disabledBtn}><span class="ascension-cost">${t('ascension.upgrade.cost')}: ${cost}</span></button></li>`;
    })
    .join('');
