...
export const ELEMENTALIST_SKILLS = {
  ...
  astralFusion: {
    id: 'astralFusion',
    name: () => t('Astral Fusion'),
    type: () => 'buff',
    manaCost: (level) => 80 + level * 2,
    cooldown: () => 90000,
    duration: () => 35000,
    requiredLevel: () => SKILL_LEVEL_TIERS[11],
    icon: () => 'astral-fusion',
    description: () => t('skill.astralFusion'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => ({
      elementalDamagePercent: scaleDownFlat(level, 6),
      manaRegenPercent: scaleDownFlat(level, 4),
      attackSpeedPercent: Math.min(scaleDownFlat(level, 0.3), 40),
    }),
  },

  celestialRend: {
    id: 'celestialRend',
    name: () => t('Celestial Rend'),
    type: () => 'instant',
    skill_type: 'spell',
    manaCost: (level) => 60 + level * 1.75,
    cooldown: () => 24000,
    requiredLevel: () => SKILL_LEVEL_TIERS[11],
    icon: () => 'celestial-rend',
    description: () => t('skill.celestialRend'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => {
      const fireDamage = scaleUpFlat(level, 18, 5);
      const coldDamage = scaleUpFlat(level, 18, 5);
      const lightningDamage = scaleUpFlat(level, 18, 5);
      const piercePercent = scaleDownFlat(level, 6);
      return {
        fireDamage: fireDamage * 4,
        coldDamage: coldDamage * 4,
        lightningDamage: lightningDamage * 4,
        elementalPenetrationPercent: Math.min(piercePercent, 25),
      };
    },
  },
  ...
  quantumFlare: {
    id: 'quantumFlare',
    name: () => t('Quantum Flare'),
    type: () => 'instant',
    skill_type: 'spell',
    manaCost: (level) => 85 + level * 2.25,
    cooldown: () => 16000,
    requiredLevel: () => SKILL_LEVEL_TIERS[12],
    icon: () => 'quantum-flare',
    description: () => t('skill.quantumFlare'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => {
      const elemental = scaleUpFlat(level, 22, 6);
      const percent = scaleDownFlat(level, 7);
      return {
        elementalDamage: elemental * 8,
        elementalDamagePercent: percent * 4,
      };
    },
  },

  stellarConvergence: {
    id: 'stellarConvergence',
    name: () => t('Stellar Convergence'),
    type: () => 'passive',
    requiredLevel: () => SKILL_LEVEL_TIERS[12],
    icon: () => 'stellar-convergence',
    description: () => t('skill.stellarConvergence'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => ({
      elementalDamagePercent: scaleDownFlat(level, 3.5),
      wisdomPercent: scaleDownFlat(level, 1.8),
      manaPercent: scaleDownFlat(level, 2.2),
      allResistance: scaleUpFlat(level, 30),
    }),
  },
  ...
  abyssalTempest: {
    id: 'abyssalTempest',
    name: () => t('Abyssal Tempest'),
    type: () => 'instant',
    skill_type: 'spell',
    manaCost: (level) => 140 + level * 3,
    cooldown: () => 30000,
    requiredLevel: () => SKILL_LEVEL_TIERS[13],
    icon: () => 'abyssal-tempest',
    description: () => t('skill.abyssalTempest'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => {
      const lightning = scaleUpFlat(level, 40, 8);
      const water = scaleUpFlat(level, 40, 8);
      const percent = scaleDownFlat(level, 8);
      return {
        lightningDamage: lightning * 8,
        waterDamage: water * 8,
        lightningDamagePercent: percent * 4,
        waterDamagePercent: percent * 4,
      };
    },
  },
  ...
  entropyHarness: {
    id: 'entropyHarness',
    name: () => t('Entropy Harness'),
    type: () => 'passive',
    requiredLevel: () => SKILL_LEVEL_TIERS[13],
    icon: () => 'entropy-harness',
    description: () => t('skill.entropyHarness'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => ({
      elementalPenetrationPercent: Math.min(scaleDownFlat(level, 0.3), 35),
      intelligencePercent: scaleDownFlat(level, 2.5),
      manaRegenOfTotalPercent: Math.min(scaleDownFlat(level, 0.015), 3),
    }),
  },
  ...
  omniElementBurst: {
    id: 'omniElementBurst',
    name: () => t('Omni-Element Burst'),
    type: () => 'instant',
    skill_type: 'spell',
    manaCost: (level) => 200 + level * 4,
    cooldown: () => 45000,
    requiredLevel: () => SKILL_LEVEL_TIERS[14],
    icon: () => 'omni-element-burst',
    description: () => t('skill.omniElementBurst'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => {
      const dmg = scaleUpFlat(level, 55, 10);
      const pct = scaleDownFlat(level, 10);
      return {
        fireDamage: dmg * 8,
        coldDamage: dmg * 8,
        lightningDamage: dmg * 8,
        earthDamage: dmg * 8,
        airDamagePercent: pct * 4,
        waterDamagePercent: pct * 4,
        fireDamagePercent: pct * 4,
        coldDamagePercent: pct * 4,
        lightningDamagePercent: pct * 4,
        earthDamagePercent: pct * 4,
      };
    },
  },

  elementalSingularity: {
    id: 'elementalSingularity',
    name: () => t('Elemental Singularity'),
    type: () => 'passive',
    requiredLevel: () => SKILL_LEVEL_TIERS[14],
    icon: () => 'elemental-singularity',
    description: () => t('skill.elementalSingularity'),
    maxLevel: () => DEFAULT_MAX_SKILL_LEVEL,
    effect: (level) => ({
      elementalDamagePercent: scaleDownFlat(level, 7),
      elementalDamage: scaleUpFlat(level, 10),
      allResistancePercent: scaleDownFlat(level, 4),
      manaPercent: scaleDownFlat(level, 4),
