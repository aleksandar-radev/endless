...
export function enemyAttack(currentTime) {
  ...
  if (game.currentEnemy.canAttack(currentTime)) {
    ...
    const isEvaded = Math.random() * 100 < hero.stats.evasion;

    if (isEvaded) {
      // Show "EVADED" text
      createDamageNumber({ text: 'EVADED', isPlayer: true, color: '#FFD700' });
    } else {
      // Check for block second
      ...
      if (isBlocked) {
        ...
        // Calculate elemental damage based on enemy element (50% of base damage)
        const elementalDamageBase = Math.floor(game.currentEnemy.damage * 0.5);
        let elementalDamage = elementalDamageBase;

        // Apply elemental resistance based on enemy element
        const enemyElement = game.currentEnemy.element;
        if (enemyElement && hero.stats[enemyElement + 'Resistance'] > 0) {
          const resistance = hero.stats[enemyElement + 'Resistance'];
          const resistanceReduction = Math.min(resistance, 75) / 100; // Cap at 75% resistance
          elementalDamage = Math.floor(elementalDamageBase * (1 - resistanceReduction));
        }
